...
[![CircleCI](https://circleci.com/gh/Raizlabs/aaa-ios/tree/develop.svg?style=shield&circle-token=ZZCIRCLE_PROJECT_STATUS_KEYZZ)][circle-ci] [![codecov](https://codecov.io/gh/Raizlabs/aaa-ios/branch/master/graph/badge.svg)](https://codecov.io/gh/Raizlabs/aaa-ios)
...
[circle-ci]: https://circleci.com/gh/Raizlabs/DemoCI-ios
[JIRA]: https://raizlabs.atlassian.net/secure/RapidBoard.jspa?projectKey=NO
...
### Setup
```bash
git clone git@github.com:Raizlabs/DemoCI-ios.git
cd DemoCI-ios
bundle install
cd app
...
`xcov` also works with Danger to provide code coverage feedback on every pull request. You can also trigger it manually. The `include_targets` filtering is to exclude external stuff like CocoaPods, but for some reason thinks the target product name for `debug-DemoCI` is `develop-DemoCI.app`. 
...
$ bundle exec xcov -w app/DemoCI.xcworkspace/ -s debug-DemoCI --include_targets "develop-DemoCI.app, Services.framework" -o app/build/xcov
```

```ruby
# Fastfile
  xcov(
    workspace: "DemoCI.xcworkspace",
    scheme: "debug-DemoCI",
    output_directory: "#{ENV['RZ_TEST_REPORTS']}/xcov",
     # For some reason coverage is on the "develop-" app target instead of "debug-"
    include_targets: "develop-DemoCI.app, Services.framework",
  )
```
...
# Dangerfile
xcov.report(
  workspace: "#{src_root}/DemoCI.xcworkspace",
  scheme: "debug-DemoCI",
  output_directory: "#{ENV['RZ_TEST_REPORTS']}/xcov",
   # For some reason coverage is on the "develop-" app target instead of "debug-"
  include_targets: "develop-DemoCI.app, Services.framework",
...
$ bundle exec slather coverage --show --html --scheme Services --workspace app/DemoCI.xcworkspace/ --output-directory app/build/slather app/DemoCI.xcodeproj/
```
